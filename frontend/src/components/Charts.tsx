  ...
  return (
    <div style={{ width: size, height: size, position: "relative", flexShrink: 0 }}>
      ...
      >
        ...
        <span style={{ fontSize: 10, color: "#8A94A6" }}>健康分</span>
      </div>
    </div>
  );
}
  ...
  const grid = [0.25, 0.5, 0.75, 1]
    .map((rt) => {
      const p = [0, 1, 2, 3]
        ...
        .join(" ");
      return <polygon key={rt} points={p} fill="none" stroke="#E4E8EE" strokeWidth={1} />;
    });
  const axes = [0, 1, 2, 3].map((i) => {
    const p = pt(i, 1);
    return (
      <line key={i} x1={cx} y1={cy} x2={p[0].toFixed(1)} y2={p[1].toFixed(1)} stroke="#E4E8EE" strokeWidth={1} />
    );
  });
  const dp = dimensions
...
export function TrendChart({
  ...
  if (vals.length === 0) return <div style={{ color: "#8A94A6", fontSize: 13 }}>暂无趋势数据</div>;
  ...
  const gridLines = [0, 25, 50, 75, 100].map((v) => (
    <g key={v}>
      <line x1={padL} y1={y(v)} x2={width - padR} y2={y(v)} stroke="#EEF1F5" strokeWidth={1} />
      <text x={padL - 6} y={y(v) + 3.5} textAnchor="end" fontSize={9.5} fill="#B4BCC8">
        {v}
      </text>
    </g>
  ));
  ...
  const levelLines = (trend.level_lines || []).map((lv: LevelConfigItem) => {
    ...
    const stroke = isRisk ? "#EF4444" : isGood ? "#0066FF" : "#94A3B8";
    return (
      <g key={lv.name}>
        <line
          x1={padL}
  ...
  const xl = labels.map((lb, i) => (
    <text key={i} x={x(i).toFixed(1)} y={height - 6} textAnchor="middle" fontSize={9.5} fill="#8A94A6">
      {lb}
    </text>
  ));
