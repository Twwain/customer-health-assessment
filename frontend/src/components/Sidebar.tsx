...
export default function Sidebar({ onNavigate }: SidebarProps) {
  ...
  const newChat = async () => {
    try {
      const s = await chat.createSession({ title: "新对话", scenario: "free_qa" });
      navigate(`/chat/${s.id}`);
      onNavigate?.();
    } catch {
      /* ignore */
  ...
  return (
    <div className="flex h-full flex-col bg-sidebar">
      ...
      <div className="px-4.5 pb-1.5 pt-2 text-[11px] font-semibold tracking-wide text-muted">
        最近会话
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-2">
        ...
        {sessions.length === 0 && (
          <div className="px-2.5 py-2 text-[12px] text-muted">暂无会话</div>
        )}
      </div>
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      <span>{label}</span>
      {badge !== undefined && (
        <span className="ml-auto flex h-[17px] min-w-[17px] items-center justify-center rounded-full bg-danger px-1 text-[10px] text-white">
          {badge}
        </span>
      )}
